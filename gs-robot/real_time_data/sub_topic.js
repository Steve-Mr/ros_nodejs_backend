 ...
 */
  ...
  * 
  * @param topic 要订阅的话题
  * @param type 订阅话题的数据类型
  * @param param （可选）关键词，只有含有此关键词才返回
 */
 router.get('/sub_topic', (req, res) => {

    let topic = req.query.topic.replace('__', "/");
   ...
   */
   let p = new Promise(function (resolve, reject) {
    ...
    //   }
       const sub = nh.subscribe(topic, type, (msg) => {
        // let msg_string = JSON.stringify(msg)
        if(param){
            if(JSON.stringify(msg).includes(param)){
